...
{{--                    <!-- Декоративная картинка -->--}}
            ...
            <div x-data="{
                ...
                fetchPosts(url) {
                    ...
                    .then(html => {
                        ...
                        this.loading = false;
                        document.getElementById('blog-posts-title').scrollIntoView({ behavior: 'smooth' });
                    })
                    .catch(error => {
            ...
            @click="let link = $event.target.closest('.blog-pagination a'); if(link) { $event.preventDefault(); fetchPosts(link.href); }">
                ...
                <div id="posts-container" :class="{ 'opacity-50 pointer-events-none': loading }" class="transition-opacity duration-300">
                    @include('public.blog.partials.posts')
                </div>
            </div>
        </div>
    </section>
