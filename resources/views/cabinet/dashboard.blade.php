        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 flex flex-col gap-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {{-- Balance Card --}}
                    <div class="relative w-full min-h-[260px] border border-cabinet-pink bg-cabinet-pink/10
            rounded-b-[10px] p-6 flex flex-col justify-between">

                        <div class="staked_card_decor absolute left-0 top-1/2 -translate-y-1/2
                h-[60%] w-[10px] bg-cabinet-pink"></div>

                        <div class="relative z-10 pl-4">
                            <div class="flex justify-between items-start mb-4">
                                <h3 class="font-semibold text-[24px] text-cabinet-text-dark">Balance</h3>

                                <div class="icon_bal_amount w-[39px] h-[54px] relative flex items-center justify-center">
                                    <div class="absolute inset-0 bg-cabinet-pink rounded-sm"></div>
                            ...
                            <div>
                                <p class="font-manrope font-extrabold text-[40px] text-cabinet-text-dark mb-4" x-text="$store.userBalance.displayAvailableBalance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' USD'">
                                    {{ number_format($user->available_balance, 2) }} USD
                                </p>
                                <div class="mt-2">

                                     <div class="w-full h-[10px] bg-cabinet-pink/20 rounded-[10px] overflow-hidden">
                                        <div class="tier-progress-bar h-full bg-cabinet-pink rounded-[10px] transition-all duration-1000 ease-out" style="width: 0%" data-progress="{{ $progressPercent }}"></div>
                                     </div>
                                    <p class="font-medium text-[16px] text-cabinet-text-grey mb-2">
                    ...
                    {{-- Amount Staked Card --}}
                    <div class="relative w-full min-h-[260px] rounded-[10px] border border-cabinet-green bg-cabinet-green/10  p-6 flex flex-col justify-between">
                        <div class="staked_card_decor absolute left-0 top-1/2 -translate-y-1/2 h-[60%] w-[7px] bg-cabinet-green rounded-r-[4px]"></div>
                        <div class="relative z-10 pl-4">
                            <div class="flex justify-between items-start mb-4">
                                <h3 class="font-semibold text-[24px] text-cabinet-text-dark">Amount Staked</h3>
                                 <div class="icon_bal_amount w-[39px] h-[54px] relative flex items-center justify-center">
                                    <div class="absolute inset-0 bg-cabinet-green rounded-sm"></div>
                            ...
                            <div>
                                <p class="font-manrope font-extrabold text-[40px] text-cabinet-text-dark mb-4">
                                    {{ number_format($totalStaked, 2) }} USD
                                </p>
                                <div class="mt-2">

                                    <div class="w-full h-[10px] bg-cabinet-green/20 rounded-[10px] overflow-hidden">
                                        <div class="tier-progress-bar h-full bg-cabinet-green rounded-[10px] transition-all duration-1000 ease-out" style="width: 0%" data-progress="{{ $progressPercent }}"></div>
                                    </div>
                                    <p class="font-medium text-[16px] text-cabinet-text-grey mb-2">
                                        {{ number_format($remainingToNextTier, 2) }} USD to reach {{ $nextTier->name ?? 'Max' }}
                                    </p>
                ...
                {{-- Investment Pools --}}
                <div class="bg-cabinet-dark border border-cabinet-grey rounded-lg p-6">
                ...
                <div class="bg-cabinet-dark border border-cabinet-grey rounded-lg p-6">
                    <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                        <div class="flex-1 w-full">
                            ...
                            <p class="text-[17px] text-cabinet-text-grey mb-3">Earn the Profits</p>
            ...
            <div class="lg:col-span-1">
                <div class="bg-cabinet-dark border border-cabinet-grey rounded-[10px] p-6 sticky top-6">

                    {{-- Profit Section --}}
                    <div class="mb-8 relative">
                        <div class="flex items-center justify-between mb-6">
                            <h3 class="text-[18px] font-semibold text-cabinet-text-dark">Profit</h3>
                            <select id="profit-period" class="bg-cabinet-dark border border-cabinet-grey text-cabinet-text-dark text-xs rounded-[4px] px-2 py-1 outline-none">
                        ...
                        <div class="relative w-[200px] h-[200px] mx-auto">
                            ...
                            <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                                <p class="text-cabinet-text-grey text-[16px] font-medium mb-0">Total</p>
                                <p class="text-cabinet-text-dark text-[20px] font-semibold" id="total-profit">${{ number_format($profitChartData['completed'] + $profitChartData['expected'], 1) }}</p>
                            </div>

                            {{-- Green Badge --}}
                             <div class="absolute top-[40%] -right-4 bg-cabinet-green text-white text-[12px] font-medium px-3 py-1 rounded-[9px] shadow-sm" id="profit-badge">
                    ...
                    <div class="pt-6 border-t border-cabinet-grey">
                        <div class="flex items-center justify-between mb-6">
                            <h3 class="text-[18px] font-semibold text-cabinet-text-dark">Calculate Profit</h3>
                            <div class="flex items-center gap-2 border border-cabinet-orange bg-cabinet-orange/10 px-3 py-1 rounded-[6px]">
                                <span id="calc-tier-name" class="text-[14px] font-bold text-cabinet-orange">
                                    {{-- Tier Name JS --}}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            {{-- Pool Selection --}}
                            <div>
                                <label class="block text-[15px] font-semibold text-cabinet-text-dark mb-2">Select Pool</label>
                                <div class="relative">
                                    <select id="calc-pool-duration" class="w-full bg-cabinet-dark border border-cabinet-grey text-cabinet-text-dark rounded-[8px] px-4 py-3 text-sm appearance-none outline-none">
                                        {{-- Populated by JS --}}
                                    </select>
                            ...
                            {{-- Amount Input --}}
                            <div>
                                <label class="block text-[15px] font-semibold text-cabinet-text-dark mb-2">Amount</label>
                                <div class="relative">
                                    <input type="number" id="calc-amount" min="0" step="100" placeholder="Enter Amount" class="w-full bg-cabinet-dark border border-cabinet-grey text-cabinet-text-dark rounded-[8px] px-4 py-3 text-sm outline-none">
                                </div>
                            </div>

                            {{-- Calculate Button --}}
                            <button id="calc-button" class="w-[200px] bg-cabinet-orange text-white py-3 rounded-[6px] font-bold text-[18px] hover:opacity-90 transition-opacity mt-4">
                                Calculate
                            </button>

                            {{-- Result --}}
                            <div class="pt-4 text-left">
                                <p class="text-[22px] font-semibold text-cabinet-text-dark inline-block mr-2">Expected Profit :</p>
                                <p id="calc-result" class="text-[22px] font-bold text-cabinet-green block mt-1"></p>
                            </div>
                        </div>
                    </div>
            ...
            profitChart = new Chart(profitCtx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        backgroundColor: ['#E87855', '#A3C1F3', 'transparent'],
                        borderWidth: 0,
                        borderRadius: 0,
                        cutout: '70%',
                    }]
